...
export interface IStorage {
  ...
  updateJobMessage(
    id: string,
    updates: Partial<InsertJobMessage>,
  ): Promise<JobMessage>;
  ...
  createWorkOrderRequest(
    request: InsertWorkOrderRequest,
  ): Promise<WorkOrderRequest>;
  getWorkOrderRequestsByClient(
    clientCompanyId: string,
  ): Promise<WorkOrderRequest[]>;
  getWorkOrderRequestsByServiceCompany(
    serviceCompanyId: string,
  ): Promise<WorkOrderRequest[]>;
  respondToWorkOrderRequest(
    requestId: string,
  ...
  // Rating operations
  createClientFieldAgentRating(
    rating: InsertClientFieldAgentRating,
  ): Promise<ClientFieldAgentRating>;
  createClientDispatcherRating(
    rating: InsertClientDispatcherRating,
  ): Promise<ClientDispatcherRating>;
  createServiceClientRating(
    rating: InsertServiceClientRating,
  ): Promise<ServiceClientRating>;
  getClientFieldAgentRating(
    workOrderId: string,
    clientId: string,
  ): Promise<ClientFieldAgentRating | undefined>;
  getClientDispatcherRating(
    workOrderId: string,
    clientId: string,
  ): Promise<ClientDispatcherRating | undefined>;
  getServiceClientRating(
    workOrderId: string,
    raterId: string,
  ): Promise<ServiceClientRating | undefined>;
  ...
  createExclusiveNetwork(
    data: InsertExclusiveNetwork,
  ): Promise<ExclusiveNetwork>;
  getExclusiveNetworks(
    clientCompanyId?: string,
    serviceCompanyId?: string,
  ): Promise<ExclusiveNetwork[]>;
  ...
  createExclusiveNetworkMember(
    member: InsertExclusiveNetworkMember,
  ): Promise<ExclusiveNetworkMember>;
  getExclusiveNetworkMembers(
    clientCompanyId: string,
  ): Promise<ExclusiveNetworkMember[]>;
  getAllExclusiveNetworkMembers(): Promise<ExclusiveNetworkMember[]>;
  updateExclusiveNetworkMember(
    id: string,
    updates: Partial<InsertExclusiveNetworkMember>,
  ): Promise<ExclusiveNetworkMember>;
  checkExclusiveNetworkEligibility(
    clientCompanyId: string,
    serviceCompanyId: string,
  ): Promise<boolean>;
  ...
  createProjectRequirement(
    requirement: InsertProjectRequirement,
  ): Promise<ProjectRequirement>;
  getProjectRequirements(projectId: string): Promise<ProjectRequirement[]>;
  updateProjectRequirement(
    id: string,
    updates: Partial<InsertProjectRequirement>,
  ): Promise<ProjectRequirement>;
  ...
  updateStructuredIssue(
    id: string,
    updates: Partial<InsertStructuredIssue>,
  ...
  createOnboardingRequest(
    request: InsertOnboardingRequest,
  ...
  updateCredential(
    id: string,
    updates: Partial<InsertCredential>,
  ): Promise<Credential>;
  ...
  updateRecognition(
    id: string,
    updates: Partial<InsertRecognition>,
  ): Promise<Recognition>;
  ...
  createWorkOrderFilter(
    filter: InsertWorkOrderFilter,
  ): Promise<WorkOrderFilter>;
  getWorkOrderFilter(workOrderId: string): Promise<WorkOrderFilter | undefined>;
  updateWorkOrderFilter(
    workOrderId: string,
    updates: Partial<InsertWorkOrderFilter>,
  ): Promise<WorkOrderFilter>;
  ...
  updateAgentRecommendation(
    id: string,
    updates: Partial<InsertAgentRecommendation>,
  ): Promise<AgentRecommendation>;
  deleteAgentRecommendation(id: string): Promise<void>;
  generateRecommendationsForWorkOrder(
    workOrderId: string,
  ): Promise<AgentRecommendation[]>;
  ...
  updateAgentSkill(
    id: string,
    updates: Partial<InsertAgentSkill>,
  ): Promise<AgentSkill>;
  ...
  updateAgentLocation(
    id: string,
    updates: Partial<InsertAgentLocation>,
  ...
  createPaymentInformation(
    paymentInfo: InsertPaymentInformation,
  ...
  updatePaymentInformation(
    id: string,
    updates: Partial<InsertPaymentInformation>,
  ): Promise<PaymentInformation>;
  ...
  createApprovedPayment(
    approvedPayment: InsertApprovedPayment,
  ...
  createBiometricAuth(
    biometricAuth: InsertBiometricAuth,
  ): Promise<BiometricAuth>;
  getBiometricAuth(
    userId: string,
    deviceId: string,
  ): Promise<BiometricAuth | undefined>;
  getBiometricAuthByCredentialId(
    credentialId: string,
  ): Promise<BiometricAuth | undefined>;
...
export class DatabaseStorage implements IStorage, IService {
  ...
  async getFieldAgents(): Promise<User[]> {
    ...
    const completionsMap = new Map(
      workOrderCompletions.map((item) => [
        item.assigneeId,
        item.completedCount,
      ]),
    );
    const issuesMap = new Map(
      unresolvedIssuesCount.map((item) => [item.reportedById, item.issueCount]),
    );
    ...
    return fieldAgents.map((agent) => {
      ...
      const company = agent.companyId
        ? companiesMap.get(agent.companyId)
        : null;
      ...
      const yearsExperience = agent.createdAt
        ? Math.max(
            1,
            Math.floor(
              (Date.now() - agent.createdAt.getTime()) /
      ...
      return {
        ...
        location:
          agent.city && agent.state ? `${agent.city}, ${agent.state}` : null,
        specializations,
        ...
        lastActive:
          agent.updatedAt?.toISOString() || agent.createdAt?.toISOString(),
      } as any;
    });
  }
  ...
  async createWorkOrder(
    workOrderData: InsertWorkOrder & { tools?: any[]; documents?: any[] },
  ): Promise<WorkOrder> {
  ...
  async getActiveTimeEntry(userId: string): Promise<any> {
    const [activeEntry] = await db
      ...
      .where(
        and(eq(timeEntries.userId, userId), eq(timeEntries.isActive, true)),
      );
    return activeEntry;
  }
  ...
  async getMessagesByRecipient(recipientId: string): Promise<Message[]> {
    return await db
      ...
      .from(messages)
      .where(
        or(eq(messages.recipientId, recipientId), isNull(messages.recipientId)),
      )
  ...
  async getDashboardStats(companyId?: string) {
    ...
    const adminCountResult = companyFilter
      ? await db
          .select({ count: count() })
          .from(users)
          .where(and(sql`'administrator' = ANY(${users.roles})`, companyFilter))
      : await db
          .select({ count: count() })
          .from(users)
          .where(sql`'administrator' = ANY(${users.roles})`);
    ...
    const managerCountResult = companyFilter
      ? await db
          .select({ count: count() })
          .from(users)
          .where(and(sql`'manager' = ANY(${users.roles})`, companyFilter))
      : await db
          .select({ count: count() })
          .from(users)
          .where(sql`'manager' = ANY(${users.roles})`);
    ...
    const dispatcherCountResult = companyFilter
      ? await db
          .select({ count: count() })
          .from(users)
          .where(and(sql`'dispatcher' = ANY(${users.roles})`, companyFilter))
      : await db
          .select({ count: count() })
          .from(users)
          .where(sql`'dispatcher' = ANY(${users.roles})`);
    ...
    const agentCountResult = companyFilter
      ? await db
          .select({ count: count() })
          .from(users)
          .where(and(sql`'field_agent' = ANY(${users.roles})`, companyFilter))
      : await db
          .select({ count: count() })
          .from(users)
    ...
    const clientCountResult = companyFilter
      ? await db
          .select({ count: count() })
          .from(users)
          .where(and(sql`'client' = ANY(${users.roles})`, companyFilter))
      : await db
          .select({ count: count() })
          .from(users)
          .where(sql`'client' = ANY(${users.roles})`);
  ...
  async getTeamReports() {
    ...
    const agentPerformance = await db
      .select({
        ...
        completedOrders: sum(
          sql<number>`CASE WHEN ${workOrders.status} = 'completed' THEN 1 ELSE 0 END`,
        ),
    ...
    const timeTrackingStats = await db
      .select({
        ...
        totalHoursWorked: sum(
          sql<number>`EXTRACT(EPOCH FROM (${timeEntries.endTime} - ${timeEntries.startTime}))/3600`,
        ),
        avgSessionLength: avg(
          sql<number>`EXTRACT(EPOCH FROM (${timeEntries.endTime} - ${timeEntries.startTime}))/3600`,
        ),
        activeEntries: sum(
          sql<number>`CASE WHEN ${timeEntries.isActive} = true THEN 1 ELSE 0 END`,
        ),
      })
      .from(timeEntries)
  ...
  async updateAssignmentProgressStatus(
    id: string,
    status: string,
  ): Promise<WorkOrder> {
    const [workOrder] = await db
  ...
  async endActiveTimeEntry(
    ...
    const [timeEntry] = await db
      ...
      .where(
        and(eq(timeEntries.id, activeEntry.id), eq(timeEntries.isActive, true)),
      )
      .returning();
  ...
  async updateWorkOrderIssue(
    id: string,
    updates: Partial<InsertWorkOrderIssue>,
  ): Promise<WorkOrderIssue> {
  ...
  async createStructuredIssue(
    issueData: InsertStructuredIssue,
  ): Promise<StructuredIssue> {
    const [issue] = await db
  ...
  async getAuditLogsForAdmin(filters?: {
    entityType?: string;
    userId?: string;
    startDate?: Date;
    endDate?: Date;
  ...
  async confirmWorkOrderNotification(
    notificationId: string,
    workOrderId: string,
  ): Promise<WorkOrder> {
  ...
  async respondToAssignmentRequest(
    ...
    const [updatedWorkOrder] = await db
      ...
      .where(
        and(eq(workOrders.id, requestId), eq(workOrders.createdById, clientId)),
      )
      .returning();
  ...
  async getWorkOrderRequestsByClient(
    clientCompanyId: string,
  ): Promise<WorkOrderRequest[]> {
    return await db
      .select({
  ...
  async getExclusiveNetworkPosts(): Promise<ExclusiveNetworkPost[]> {
    return await db
      ...
      .leftJoin(
        companies,
        eq(exclusiveNetworkPosts.clientCompanyId, companies.id),
      )
  ...
  // Rating operations
  async createClientFieldAgentRating(
    rating: InsertClientFieldAgentRating,
  ): Promise<ClientFieldAgentRating> {
    const [newRating] = await db
      .insert(clientFieldAgentRatings)
      .values(rating)
      .returning();
  ...
  async createClientDispatcherRating(
    rating: InsertClientDispatcherRating,
  ): Promise<ClientDispatcherRating> {
    const [newRating] = await db
      .insert(clientDispatcherRatings)
      .values(rating)
      .returning();
  ...
  async createServiceClientRating(
    rating: InsertServiceClientRating,
  ): Promise<ServiceClientRating> {
    const [newRating] = await db
  ...
  async getClientFieldAgentRating(
    workOrderId: string,
    clientId: string,
  ): Promise<ClientFieldAgentRating | undefined> {
  ...
  async getClientDispatcherRating(
    workOrderId: string,
    clientId: string,
  ): Promise<ClientDispatcherRating | undefined> {
    const [rating] = await db
  ...
  async updateUserRatings(userId: string): Promise<void> {
    ...
    if (fieldAgentRatings.length > 0) {
      const avgCommunication =
        fieldAgentRatings.reduce((sum, r) => sum + r.communicationRating, 0) /
        fieldAgentRatings.length;
      const avgTimeliness =
        fieldAgentRatings.reduce((sum, r) => sum + r.timelinessRating, 0) /
        fieldAgentRatings.length;
      const avgWorkSatisfaction =
        fieldAgentRatings.reduce(
          (sum, r) => sum + r.workSatisfactionRating,
          0,
        ) / fieldAgentRatings.length;
      const overallRating =
        (avgCommunication + avgTimeliness + avgWorkSatisfaction) / 3;
    ...
    if (dispatcherRatings.length > 0) {
      const avgCommunication =
        dispatcherRatings.reduce((sum, r) => sum + r.communicationRating, 0) /
        dispatcherRatings.length;
      const avgManagement =
        dispatcherRatings.reduce((sum, r) => sum + r.managementRating, 0) /
        dispatcherRatings.length;
      const avgFieldAgent =
        dispatcherRatings.reduce((sum, r) => sum + r.fieldAgentRating, 0) /
        dispatcherRatings.length;
      const overallRating =
        (avgCommunication + avgManagement + avgFieldAgent) / 3;
    ...
    if (clientRatings.length > 0) {
      const avgCommunication =
        clientRatings.reduce((sum, r) => sum + r.communicationRating, 0) /
        clientRatings.length;
      const avgClearScope =
        clientRatings.reduce((sum, r) => sum + r.clearScopeRating, 0) /
        clientRatings.length;
      const avgOverallSatisfaction =
        clientRatings.reduce((sum, r) => sum + r.overallSatisfactionRating, 0) /
        clientRatings.length;
      const overallRating =
        (avgCommunication + avgClearScope + avgOverallSatisfaction) / 3;
  ...
  async updateCompanyRatings(companyId: string): Promise<void> {
    ...
    const companyUsers = await db
      ...
      .from(users)
      .where(eq(users.companyId, companyId));
    const usersWithRatings = companyUsers.filter(
      (user) => user.totalRatings && user.totalRatings > 0,
    );

    if (usersWithRatings.length > 0) {
      const totalRatingSum = usersWithRatings.reduce(
        (sum, user) => sum + (user.overallRating || 0),
        0,
      );
      const totalRatingsCount = usersWithRatings.reduce(
        (sum, user) => sum + (user.totalRatings || 0),
        0,
      );
  ...
  async getAllFeedback(filters: {
    ...
    if (filters.companyId) {
      ...
      const feedbackJoin = leftJoin(
        feedback,
        eq(feedback.workOrderId, workOrders.id),
      );
    ...
    if (filters.dateFrom) {
      conditions.push(
        sql`${feedback.createdAt} >= ${new Date(filters.dateFrom)}`,
      );
    }

    if (filters.dateTo) {
      conditions.push(
        sql`${feedback.createdAt} <= ${new Date(filters.dateTo)}`,
      );
    }
  ...
  async calculateAgentPerformanceMetrics(
    ...
    const completedJobs = completedWorkOrders.filter(
      (wo) => wo.status === "completed",
    ).length;

    // Calculate average completion time
    const completedWithActualHours = completedWorkOrders.filter(
      (wo) => wo.actualHours && wo.status === "completed",
    );
    const avgCompletionTime =
      completedWithActualHours.length > 0
        ? completedWithActualHours.reduce(
            (sum, wo) => sum + (wo.actualHours || 0),
            0,
          ) / completedWithActualHours.length
        : 0;
    ...
    const onTimeStartPercentage =
      totalJobs > 0 ? (onTimeStarts / totalJobs) * 100 : 0;
    const onTimeFinishPercentage =
      totalJobs > 0 ? (onTimeFinishes / totalJobs) * 100 : 0;
    ...
    const avgStarRating =
      feedbackRatings.length > 0
        ? feedbackRatings.reduce((sum, rating) => sum + rating.stars, 0) /
          feedbackRatings.length
        : 0;

    const wouldHireAgainCount = feedbackRatings.filter(
      (rating) => rating.wouldHireAgain === true,
    ).length;
    ...
    const issuesCount = await db
      ...
      .where(
        and(eq(workOrders.assignedAgent, agentId), ...conditions.slice(1)),
      );
    ...
    const complianceScore =
      completedJobs > 0 ? auditLogCount / completedJobs : 0;
    ...
    return {
      ...
      wouldHireAgainPercentage:
        Math.round(wouldHireAgainPercentage * 100) / 100,
      totalIssues,
      resolvedIssues,
  ...
  async createServiceQualitySnapshot(
    snapshot: InsertServiceQualitySnapshot,
  ): Promise<ServiceQualitySnapshot> {
    const [newSnapshot] = await db
  ...
  async getServiceQualitySnapshots(
    companyId: string,
    limit: number = 10,
  ): Promise<ServiceQualitySnapshot[]> {
    return await db
  ...
  async calculateServiceQualityMetrics(
    companyId: string,
    dateFrom?: string,
    dateTo?: string,
  ): Promise<any> {
    ...
    const completedJobs = companyWorkOrders.filter(
      (wo) => wo.status === "completed",
    ).length;
    ...
    const recentJobs = companyWorkOrders.filter(
    ...
    const onTimeFinishPercentage =
      totalJobs > 0 ? (onTimeFinishes / totalJobs) * 100 : 0;
    const serviceComplianceScore =
      (onTimeStartPercentage + onTimeFinishPercentage) / 2;
    ...
    if (agentIds.length > 0) {
      ...
      const filteredFeedback = feedbackData.filter((f) => {
        if (dateFrom && new Date(f.createdAt) < new Date(dateFrom))
          return false;
      ...
      wouldHireAgainCount = filteredFeedback.filter(
        (f) => f.wouldHireAgain === true,
      ).length;
    }

    const avgClientSatisfaction =
      totalFeedback > 0 ? totalStarsSum / totalFeedback : 0;
    const wouldHireAgainPercentage =
      totalFeedback > 0 ? (wouldHireAgainCount / totalFeedback) * 100 : 0;
    const overallClientSatisfactionScore =
      totalFeedback > 0
    ...
    if (workOrderIds.length > 0) {
      ...
      resolvedIssues = issuesData.filter(
        (issue) => issue.status === "resolved",
      ).length;

      // Calculate average resolution time for resolved issues
      const resolvedIssuesWithTime = issuesData.filter(
        (issue) => issue.status === "resolved" && issue.reviewedAt,
      );
    ...
    const issueRate =
      completedJobs > 0 ? (totalIssues / completedJobs) * 100 : 0;
    const issueResolutionRate =
      totalIssues > 0 ? (resolvedIssues / totalIssues) * 100 : 0;
    ...
    return {
      ...
      totalJobs,
      completedJobs,
      jobVolumeData: Array.from(jobVolumeByDay.entries()).map(
        ([date, count]) => ({
          date,
          count,
        }),
      ),
      overallClientSatisfactionScore:
        Math.round(overallClientSatisfactionScore * 100) / 100,
  ...
  // Exclusive Network operations
  async createExclusiveNetworkMember(
    member: InsertExclusiveNetworkMember,
  ): Promise<ExclusiveNetworkMember> {
    const [newMember] = await db
      .insert(exclusiveNetworkMembers)
      .values(member)
      .returning();
    return newMember;
  }

  async getExclusiveNetworkMembers(
    clientCompanyId: string,
  ): Promise<ExclusiveNetworkMember[]> {
    return await db
      .select()
  ...
  async getAllExclusiveNetworkMembers(): Promise<ExclusiveNetworkMember[]> {
    ...
    return await db
      ...
      .leftJoin(
        clientCompanies,
        eq(exclusiveNetworkMembers.clientCompanyId, clientCompanies.id),
      )
      .leftJoin(
        serviceCompanies,
        eq(exclusiveNetworkMembers.serviceCompanyId, serviceCompanies.id),
      )
      .orderBy(desc(exclusiveNetworkMembers.createdAt));
  }

  async updateExclusiveNetworkMember(
    id: string,
    updates: Partial<InsertExclusiveNetworkMember>,
  ): Promise<ExclusiveNetworkMember> {
  ...
  async checkExclusiveNetworkEligibility(
    clientCompanyId: string,
    serviceCompanyId: string,
  ): Promise<boolean> {
  ...
  async requestProjectAssignment(
    projectId: string,
    companyId: string,
    requestedById: string,
  ): Promise<ProjectAssignment> {
  ...
  async createProjectRequirement(
    requirementData: InsertProjectRequirement,
  ): Promise<ProjectRequirement> {
  ...
  async createAccessRequest(
    requestData: InsertAccessRequest,
  ): Promise<AccessRequest> {
    const [request] = await db
  ...
  async getExclusiveNetworks(
    ...
    let query = db
      ...
      .where(eq(exclusiveNetworks.isActive, true));
    ...
    if (serviceCompanyId) {
      query = query.where(
        eq(exclusiveNetworks.serviceCompanyId, serviceCompanyId),
      );
    }
  ...
  async getExclusiveServiceCompanies(
    clientCompanyId: string,
  ): Promise<Company[]> {
    const result = await db
      .select({
      ...
      .innerJoin(
        companies,
        eq(exclusiveNetworks.serviceCompanyId, companies.id),
      )
      .where(
        and(
          eq(exclusiveNetworks.clientCompanyId, clientCompanyId),
  ...
  async calculateProfit(workOrderId: string): Promise<WorkOrder> {
    ...
    const estimatedHours = parseFloat(
      workOrder.estimatedHours?.toString() || "0",
    );
    ...
    const profitMargin =
      totalBudget > 0 ? (actualProfit / totalBudget) * 100 : 0;
  ...
  async reviewOnboardingRequest(
    ...
    if (status === "approved" && request) {
      ...
      const assignedCompanyId =
        serviceCompanies.length > 0 ? serviceCompanies[0].id : null;
  ...
  async getRiskScores(
    ...
    return await query.orderBy(desc(riskScores.createdAt)).limit(limit);
  }
  ...
  async calculateRiskScore(
    ...
    if (entityType === "agent") {
      ...
      const agentMetrics = await this.calculateAgentPerformanceMetrics(
        entityId,
        periodStart,
        periodEnd,
      );
      ...
      const avgTimelinessScore =
        (agentMetrics.onTimeStartPercentage +
          agentMetrics.onTimeFinishPercentage) /
        2;
      ...
      const companyMetrics = await this.calculateServiceQualityMetrics(
        entityId,
        periodStart,
        periodEnd,
      );
      ...
      if (companyMetrics.avgClientSatisfaction < 3.5) {
        clientSatisfactionScore =
          (companyMetrics.avgClientSatisfaction / 5) * 100;
        flaggedMetrics.clientSatisfaction = {
          current: companyMetrics.avgClientSatisfaction,
          threshold: 3.5,
          severity:
            companyMetrics.avgClientSatisfaction < 2.5 ? "high" : "medium",
        };
      }
      ...
      if (companyMetrics.serviceComplianceScore < 80) {
        ...
        flaggedMetrics.timeliness = {
          ...
          threshold: 80,
          severity:
            companyMetrics.serviceComplianceScore < 60 ? "high" : "medium",
        };
      }
      ...
      if (companyMetrics.auditComplianceScore < 0.8) {
        ...
        flaggedMetrics.compliance = {
          ...
          threshold: 0.8,
          severity:
            companyMetrics.auditComplianceScore < 0.6 ? "high" : "medium",
        };
      }
  ...
  async createRiskIntervention(
    intervention: InsertRiskIntervention,
  ...
  async updateRiskIntervention(
    id: string,
    updates: Partial<InsertRiskIntervention & { completedAt?: Date }>,
  ): Promise<RiskIntervention> {
    const [updated] = await db
  ...
  async createWorkOrderTool(tool: InsertWorkOrderTool): Promise<WorkOrderTool> {
    const [newTool] = await db.insert(workOrderTools).values(tool).returning();
    return newTool;
  }
  ...
  async deleteWorkOrderTool(id: string): Promise<void> {
    await db.delete(workOrderTools).where(eq(workOrderTools.id, id));
  }

  async confirmToolAvailability(
    id: string,
    confirmedById: string,
  ): Promise<WorkOrderTool> {
    const [updated] = await db
  ...
  async getBidsForWorkOrder(workOrderId: string): Promise<Bid[]> {
    return await db
      .select()
      .from(bids)
      .where(eq(bids.workOrderId, workOrderId))
  ...
  async createCredential(
    credentialData: InsertCredential,
  ): Promise<Credential> {
  ...
  async getUserRecognition(userId: string): Promise<Recognition[]> {
    return await db
      .select()
      .from(recognition)
      .where(eq(recognition.userId, userId))
  ...
  > {
    const results = await db
      ...
      .from(workOrders)
      .where(
        and(isNotNull(workOrders.category), eq(workOrders.status, "completed")),
      )
  ...
  async getWorkOrderFilter(
    workOrderId: string,
  ): Promise<WorkOrderFilter | undefined> {
  ...
  async updateAgentRecommendation(
    id: string,
    updates: Partial<InsertAgentRecommendation>,
  ): Promise<AgentRecommendation> {
  ...
  async generateRecommendationsForWorkOrder(
    workOrderId: string,
  ): Promise<AgentRecommendation[]> {
  ...
  // Agent Location operations (Module 3)
  async createAgentLocation(
    location: InsertAgentLocation,
  ): Promise<AgentLocation> {
  ...
  async getPrimaryAgentLocation(
    agentId: string,
  ): Promise<AgentLocation | undefined> {
    const [location] = await db
      ...
      .where(
        and(
          ...
          eq(agentLocations.isPrimary, true),
        ),
      );
    return location;
  }

  async getAgentPrimaryLocation(
    agentId: string,
  ): Promise<AgentLocation | undefined> {
  ...
  async deleteAgentLocation(id: string): Promise<void> {
    await db.delete(agentLocations).where(eq(agentLocations.id, id));
  }
  ...
  async searchWorkOrdersWithFilters(filters: {
    ...
    if (filters.minPayRate) {
      conditions.push(
        gte(sql`CAST(${workOrders.budget} AS NUMERIC)`, filters.minPayRate),
      );
    }
    if (filters.maxPayRate) {
      conditions.push(
        lte(sql`CAST(${workOrders.budget} AS NUMERIC)`, filters.maxPayRate),
      );
    }
    ...
    if (filters.experienceRequired) {
      conditions.push(
        ilike(workOrders.description, `%${filters.experienceRequired}%`),
      );
    }
    ...
    if (filters.equipmentProvided !== undefined) {
      conditions.push(
        eq(workOrders.equipmentProvided, filters.equipmentProvided),
      );
    }
    ...
    if (filters.agentLocation && filters.locationRadius) {
      ...
      const lngRange =
        filters.locationRadius / (111 * Math.cos((lat * Math.PI) / 180));
  ...
  async getVisibleJobsForAgent(filters: {
      ...
      switch (filters.agentExperienceLevel) {
        case "junior":
          ...
          experienceConditions.push(
            inArray(workOrders.priority, ["low", "medium"]),
          );
          break;
        case "intermediate":
          // Intermediate agents can see all except urgent without prior approval
          experienceConditions.push(
            inArray(workOrders.priority, ["low", "medium", "high"]),
          );
          break;
        case "senior":
    ...
    if (filters.agentLocation && filters.radiusKm > 0) {
      ...
      const lngRange =
        filters.radiusKm / (111 * Math.cos((lat * Math.PI) / 180));
  ...
  async createProjectHeartbeat(
    heartbeat: InsertProjectHeartbeat,
  ...
  async getProjectHeartbeat(
    workOrderId: string,
  ): Promise<ProjectHeartbeat | undefined> {
  ...
  async updateProjectHeartbeat(
    id: string,
    updates: Partial<InsertProjectHeartbeat>,
  ): Promise<ProjectHeartbeat> {
  ...
  async deleteProjectHeartbeat(id: string): Promise<void> {
    await db.delete(projectHeartbeats).where(eq(projectHeartbeats.id, id));
  }

  async createHeartbeatEvent(
    event: InsertHeartbeatEvent,
  ...
  async updateHealthScore(
    ...
    await this.createHeartbeatEvent({
      ...
      impact:
        newScore >= 80 ? "positive" : newScore >= 60 ? "neutral" : "negative",
  ...
  async triggerHeartbeatEvent(
    ...
    if (healthScoreChange !== 0) {
      const newScore = Math.max(
        0,
        Math.min(100, heartbeat.healthScore + healthScoreChange),
      );
  ...
  async getStructuredIssues(filters: any = {}): Promise<Issue[]> {
    let query = db
      ...
      .leftJoin(
        ...
        sql`${issues.reportedById} = reporter.id`,
      )
      .leftJoin(
        sql`work_orders AS work_order`,
        sql`${issues.workOrderId} = work_order.id`,
      )
      .leftJoin(
        sql`users AS resolver`,
        sql`${issues.resolvedById} = resolver.id`,
      );
  ...
  async getIssueAnalytics(params: {
    period: number;
    companyId?: string;
  }): Promise<any> {
  ...
  async getFeedbackAnalytics(params: {
    ...
    if (params.companyId) {
      fieldAgentQuery = fieldAgentQuery.where(
        eq(clientFieldAgentRatings.companyId, params.companyId),
      );
    }

    if (params.agentId) {
      fieldAgentQuery = fieldAgentQuery.where(
        eq(clientFieldAgentRatings.fieldAgentId, params.agentId),
      );
    }
      ...
      .where(
        and(
          ...
          params.companyId
            ...
            : undefined,
          params.agentId
            ? eq(workOrders.assigneeId, params.agentId)
            : undefined,
        ).filter(Boolean),
      );

    const responseRate =
      totalWorkOrders[0]?.count > 0
        ? (totalRatings / totalWorkOrders[0].count) * 100
        : 0;

    // Calculate average ratings
    const avgCommunication =
      fieldAgentRatings.length > 0
        ? fieldAgentRatings.reduce((sum, r) => sum + r.communicationRating, 0) /
          fieldAgentRatings.length
        : 0;
    ...
    const highRatings = fieldAgentRatings.filter(
      (r) =>
        r.communicationRating >= 4 &&
        r.timelinessRating >= 4 &&
        r.workSatisfactionRating >= 4,
    ).length;
    const satisfactionScore =
      totalRatings > 0 ? (highRatings / totalRatings) * 100 : 0;
    ...
    const categoryBreakdown = [
      ...
      { name: "Timeliness", rating: avgTimeliness, count: totalRatings },
      {
        ...
        rating: avgWorkSatisfaction,
        count: totalRatings,
      },
    ];

    // Recent feedback (simplified)
    const recentFeedback = fieldAgentRatings.slice(0, 5).map((rating) => ({
    ...
    if (avgCommunication < 3.5) {
      actionItems.push({
        ...
        description:
          "Communication rating is below threshold. Consider additional training.",
      });
    }
    if (avgTimeliness < 3.5) {
      actionItems.push({
        ...
        description:
          "Timeliness rating needs attention. Review scheduling processes.",
      });
    }
    if (avgWorkSatisfaction < 3.5) {
      actionItems.push({
        ...
        description:
          "Work satisfaction scores indicate need for quality improvements.",
      });
    }
  ...
  async sendBulkFeedbackRequests(
    workOrderIds: string[],
    reminderType: string,
    requesterId: string,
  ): Promise<any> {
    ...
    for (const workOrderId of workOrderIds) {
      try {
        ...
        console.error(
          `Failed to send feedback request for work order ${workOrderId}:`,
          error,
        );
        results.failed++;
      }
  ...
  async getJobRequests(params: {
    ...
    let query = db
      ...
      .from(jobNetworkPosts)
      .leftJoin(
        sql`companies AS company`,
        sql`${jobNetworkPosts.clientCompanyId} = company.id`,
      )
      .leftJoin(
        sql`users AS posted_by`,
        sql`${jobNetworkPosts.postedById} = posted_by.id`,
      );
  ...
  async getContractorOnboardingDetails(
    onboardingId: string,
    requesterId: string,
  ): Promise<any> {
  ...
  async createContractorOnboarding(onboardingData: any): Promise<any> {
    ...
    await db.insert(notifications).values({
      ...
      message:
        "Your contractor onboarding application has been submitted for review.",
    });
  ...
  async processOnboardingApproval(
    onboardingId: string,
    ...
    if (record[0]) {
      ...
      await db.insert(notifications).values({
        ...
        type:
          action === "approve" ? "onboarding_approved" : "onboarding_rejected",
        title:
          action === "approve" ? "Onboarding Approved" : "Onboarding Rejected",
        message:
          action === "approve"
  ...
  async getMessagingChannels(params: {
    ...
    let query = db
      ...
      .leftJoin(
        sql`channel_messages AS messages`,
        sql`${messagingChannels.id} = messages.channel_id`,
      )
      .groupBy(
    ...
    if (
      !["operations_director", "administrator"].includes(params.userRole || "")
    ) {
    ...
    return await query.orderBy(
      desc(messagingChannels.lastActivityAt),
      desc(messagingChannels.createdAt),
    );
  }
  ...
  async getChannelMessages(channelId: string, userId: string): Promise<any[]> {
    ...
    const messages = await db
      ...
      .leftJoin(
        sql`users AS sender`,
        sql`${channelMessages.senderId} = sender.id`,
      )
      .leftJoin(
        sql`channel_messages AS reply_to`,
        sql`${channelMessages.replyToId} = reply_to.id`,
      )
      .leftJoin(
        sql`users AS reply_sender`,
        sql`reply_to.sender_id = reply_sender.id`,
      )
  ...
  async getProjectHeartbeatEvents(
    heartbeatId: string,
    limit: number = 50,
    offset: number = 0,
  ): Promise<any[]> {
  ...
  async createProjectHealthLog(logData: any): Promise<any> {
    const [log] = await db.insert(projectHealthLog).values(logData).returning();
    return log;
  }
  ...
  async getUserPaymentMethods(userId: string): Promise<PaymentInformation[]> {
    return await db
      ...
      .where(
        and(
          eq(paymentInformation.userId, userId),
          eq(paymentInformation.isActive, true),
        ),
      )
      .orderBy(
        desc(paymentInformation.isDefault),
        desc(paymentInformation.createdAt),
      );
  }
  ...
  async getCompanyPaymentMethods(
    ...
    return await db
      ...
      .where(
        and(
          eq(paymentInformation.companyId, companyId),
          eq(paymentInformation.isActive, true),
        ),
      )
      .orderBy(
  ...
  async getUserTransactions(userId: string): Promise<PaymentTransaction[]> {
    return await db
      ...
      .where(
        or(
          eq(paymentTransactions.fromUserId, userId),
          eq(paymentTransactions.toUserId, userId),
        ),
      )
      .orderBy(desc(paymentTransactions.createdAt));
  }

  async getWorkOrderTransactions(
    workOrderId: string,
  ): Promise<PaymentTransaction[]> {
    return await db
      .select()
  ...
  async getTransactionHistory(filters: {
    ...
    if (filters.userId) {
      conditions.push(
        or(
          eq(paymentTransactions.fromUserId, filters.userId),
          eq(paymentTransactions.toUserId, filters.userId),
        ),
      );
    }
    if (filters.companyId) {
      conditions.push(
        or(
          eq(paymentTransactions.fromCompanyId, filters.companyId),
          eq(paymentTransactions.toCompanyId, filters.companyId),
        ),
      );
    }
    if (filters.status) {
  ...
  async getAvailableApprovedPayments(
    userId: string,
  ): Promise<ApprovedPayment[]> {
    return await db
      .select()
  ...
  async getApprovedPaymentById(
    id: string,
  ): Promise<ApprovedPayment | undefined> {
    const [payment] = await db
      .select()
      .from(approvedPayments)
      .where(eq(approvedPayments.id, id));
  ...
  // Expense Order operations
  async createExpenseOrder(
    expenseOrder: InsertExpenseOrder,
  ): Promise<ExpenseOrder> {
    const [expense] = await db
      .insert(expenseOrders)
      .values(expenseOrder)
      .returning();
    return expense;
  }
  ...
  async getUserExpenses(userId: string): Promise<ExpenseOrder[]> {
    return await db
      ...
      .where(
        or(
          eq(expenseOrders.createdById, userId),
          eq(expenseOrders.assignedUserId, userId),
        ),
      )
      .orderBy(desc(expenseOrders.createdAt));
  }

  async approveExpenseOrder(
    id: string,
    approvedById: string,
  ): Promise<ExpenseOrder> {
    const [expense] = await db
  ...
  async revokeDeviceToken(userId: string, deviceId: string): Promise<void> {
    await db
      ...
      .set({ isActive: false })
      .where(
        and(eq(deviceTokens.userId, userId), eq(deviceTokens.id, deviceId)),
      );
  }
  ...
  // Biometric Authentication operations
  async createBiometricAuth(
    biometricAuthData: InsertBiometricAuth,
  ): Promise<BiometricAuth> {
    const [auth] = await db
      .insert(biometricAuth)
      .values(biometricAuthData)
      .returning();
    return auth;
  }
  ...
  async getBiometricAuthByCredentialId(
    credentialId: string,
  ): Promise<BiometricAuth | undefined> {
    const [auth] = await db
      .select()
  ...
  async getUserBiometricMethods(userId: string): Promise<BiometricAuth[]> {
    return await db
      ...
      .from(biometricAuth)
      .where(
        and(eq(biometricAuth.userId, userId), eq(biometricAuth.isActive, true)),
      )
